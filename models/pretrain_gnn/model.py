...
class MaskGAE(nn.Module):
    def __init__(
        ...
        self.node_decoder = nn.Linear(input_dim, input_dim, bias=False)
    ...
    def train_step(self, data, optimizer, alpha=0.002, beta=0.001, grad_norm=1.0):
        self.train()

        x, edge_index = data.x, data.edge_index
        internal_edge_index = data.internal_edge_index
        num_node = x.size(0)
        batch_size = int(x.size(0) / self.num_node)

        # EdgeMask
        if self.edge_p < 0. or self.edge_p > 1.:
            raise ValueError(f'Edge mask probability has to be between 0 and 1 '
                            f'(got {self.edge_p}')    
        edge_ids = torch.arange(edge_index.size(1), dtype=torch.long, device=edge_index.device)
        mask = torch.full_like(edge_ids, self.edge_p, dtype=torch.float32)
        mask = torch.bernoulli(mask).to(torch.bool)
        remaining_edges = edge_index[:, ~mask]
        masked_edges = edge_index[:, mask]
        
        # NodeMask
        if self.node_p < 0. or self.node_p > 1.:
            raise ValueError(f'Node mask probability has to be between 0 and 1 '
                            f'(got {self.node_p}')
        num_nodes = x.size(0)
        perm = torch.randperm(num_nodes, device=x.device)
        num_mask_nodes = int(num_nodes * self.node_p)
        ...
        for perm in progress_bar:
            ...
            ### Edge mask with edge reconstruction
            ...
            deg_loss = F.mse_loss(self.degree_decoder(z).squeeze(), deg)
            # **********************************************************************

            ### Node mask with node reconstruction
            use_x = x.clone()
            use_x[mask_nodes] = 0.0
            use_z = self.internal_encoder(use_x, internal_edge_index)
            use_z = self.encoder(use_z, edge_index)

            # ******************* loss for node reconstruction *********************
            rec_z = self.node_decoder(use_z)
            init_x = x[mask_nodes]
            rec_x = rec_z[mask_nodes]
            node_loss = F.mse_loss(rec_x, init_x)
            # **********************************************************************

            loss = edge_loss + alpha * deg_loss + beta * node_loss
    ...
    def test_step(self, data, pos_edge_index, neg_edge_index, batch_size=2**16):
        ...
        # # Count correct predictions based on a threshold
        # threshold = 0.5
        # pred_binary = (pred >= threshold).astype(int)
        # correct_count = (pred_binary == y).sum()
